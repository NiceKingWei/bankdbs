...
trait Tables {
   ...
   *  @param departmentNum Database column department_num SqlType(DECIMAL), PrimaryKey
   *  @param bankName Database column bank_name SqlType(CHAR), Length(64,false)
   *  @param staffIdCard Database column staff_id_card SqlType(CHAR), Length(255,false)
   *  @param name Database column name SqlType(CHAR), Length(64,false) */
  case class DepartmentRow(departmentNum: scala.math.BigDecimal, bankName: String, staffIdCard: String, name: String)
  /** GetResult implicit for fetching DepartmentRow objects using plain SQL queries */
  implicit def GetResultDepartmentRow(implicit e0: GR[scala.math.BigDecimal], e1: GR[String]): GR[DepartmentRow] = GR{
    prs => import prs._
    DepartmentRow.tupled((<<[scala.math.BigDecimal], <<[String], <<[String], <<[String]))
  }
  /** Table description of table department. Objects of this class serve as prototypes for rows in queries. */
  class Department(_tableTag: Tag) extends profile.api.Table[DepartmentRow](_tableTag, Some("bankdb"), "department") {
    def * = (departmentNum, bankName, staffIdCard, name) <> (DepartmentRow.tupled, DepartmentRow.unapply)
    /** Maps whole row to an option. Useful for outer joins. */
    def ? = (Rep.Some(departmentNum), Rep.Some(bankName), Rep.Some(staffIdCard), Rep.Some(name)).shaped.<>({r=>import r._; _1.map(_=> DepartmentRow.tupled((_1.get, _2.get, _3.get, _4.get)))}, (_:Any) =>  throw new Exception("Inserting into ? projection not supported."))

    /** Database column department_num SqlType(DECIMAL), PrimaryKey */
    val departmentNum: Rep[scala.math.BigDecimal] = column[scala.math.BigDecimal]("department_num", O.PrimaryKey)
    /** Database column bank_name SqlType(CHAR), Length(64,false) */
    val bankName: Rep[String] = column[String]("bank_name", O.Length(64,varying=false))
    /** Database column staff_id_card SqlType(CHAR), Length(255,false) */
    val staffIdCard: Rep[String] = column[String]("staff_id_card", O.Length(255,varying=false))
    /** Database column name SqlType(CHAR), Length(64,false) */
    val name: Rep[String] = column[String]("name", O.Length(64,varying=false))
   ...
   *  @param departmentNum Database column department_num SqlType(DECIMAL)
   *  @param name Database column name SqlType(CHAR), Length(64,false)
   *  @param phone Database column phone SqlType(CHAR), Length(16,false)
   *  @param address Database column address SqlType(TEXT)
   *  @param startDate Database column start_date SqlType(DATE) */
  case class StaffRow(staffIdCard: String, departmentNum: scala.math.BigDecimal, name: String, phone: String, address: String, startDate: java.sql.Date)
  /** GetResult implicit for fetching StaffRow objects using plain SQL queries */
  implicit def GetResultStaffRow(implicit e0: GR[String], e1: GR[scala.math.BigDecimal], e2: GR[java.sql.Date]): GR[StaffRow] = GR{
    prs => import prs._
    StaffRow.tupled((<<[String], <<[scala.math.BigDecimal], <<[String], <<[String], <<[String], <<[java.sql.Date]))
  }
  ...
  class Staff(_tableTag: Tag) extends profile.api.Table[StaffRow](_tableTag, Some("bankdb"), "staff") {
    def * = (staffIdCard, departmentNum, name, phone, address, startDate) <> (StaffRow.tupled, StaffRow.unapply)
    /** Maps whole row to an option. Useful for outer joins. */
    def ? = (Rep.Some(staffIdCard), Rep.Some(departmentNum), Rep.Some(name), Rep.Some(phone), Rep.Some(address), Rep.Some(startDate)).shaped.<>({r=>import r._; _1.map(_=> StaffRow.tupled((_1.get, _2.get, _3.get, _4.get, _5.get, _6.get)))}, (_:Any) =>  throw new Exception("Inserting into ? projection not supported."))
    ...
    /** Database column department_num SqlType(DECIMAL) */
    val departmentNum: Rep[scala.math.BigDecimal] = column[scala.math.BigDecimal]("department_num")
    /** Database column name SqlType(CHAR), Length(64,false) */
    val name: Rep[String] = column[String]("name", O.Length(64,varying=false))
    /** Database column phone SqlType(CHAR), Length(16,false) */
    val phone: Rep[String] = column[String]("phone", O.Length(16,varying=false))
    ...
    /** Foreign key referencing Department (database name FK_staff_department_relationship) */
    lazy val departmentFk = foreignKey("FK_staff_department_relationship", departmentNum, Department)(r => r.departmentNum, onUpdate=ForeignKeyAction.NoAction, onDelete=ForeignKeyAction.NoAction)
  }
